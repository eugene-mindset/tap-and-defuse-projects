...
def store_matches(matches: list[Match], rel_path=''):
    ...
    map_matches =  [(match.id_num, i, map_match) 
        for i, map_match in enumerate(match.maps) for match in matches]
    ...
    stats_matches = [(match.id_num, i, stats)
        for i, stats in enumerate(match.player_stats) for match in matches]

    with open(out_path / 'stats_maps.csv', 'w', newline='', encoding="utf8") as file:
        writer = csv_writer(file)
        writer.writerow(['ID', 'Order', 'Map', 'Team', 'PlayerID', 'Agent'])
        ...
        for id_num, order, map_stats in maps_tqdm:
            for i, team in enumerate(map_stats):
                for player_id, stats in team.items():
                    writer.writerow([id_num, order, map_match, i, player_id, str(stats['agent'])[7:]])
...
def _test():
    ...
    test_match.player_stats = [[{'0001': {'agent': Agents.ASTRA}}, {'0002': {'agent': Agents.BREACH}}]]
    ...
    store_matches([test_match], rel_path=test_dir)
